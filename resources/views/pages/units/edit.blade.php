@extends('layouts.app', ['activePage' => 'units', 'titlePage' => __('Edit Unit')])
...
@section('content')
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <form method="post" action="{{ route('units.update', ['id' => $unit['id']]) }}" autocomplete="off" class="form-horizontal">
                        @csrf

                        <div class="card ">
                            <div class="card-header card-header-primary">
                                <h4 class="card-title">{{ __('Edit Unit') }}</h4>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('unit') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('unit') ? ' is-invalid' : '' }}" name="unit" id="input-unit" type="text" placeholder="{{ __('Unit') }}" value="{{ old('unit', $unit['unit']) }}" required="true" aria-required="true"/>
                                            @if ($errors->has('unit'))
                                        ...
                                        <div class="{{ $errors->has('grade') ? ' has-danger' : '' }}">
                                            <select class="{{ $errors->has('grade') ? ' is-invalid' : '' }}" name="grade" data-live-search="true" id="input-grade">
                                                @foreach($grades as $grade)
                                                    <option {{$grade['id'] == $unit['grade_id'] ? "selected" : ""}} value="{{$grade['id']}}">{{$grade['grade']}}</option>
                                                @endforeach
                                            </select>
                                            @if ($errors->has('grade'))
